...
onMounted(() => {
  ...
  addFrameCallback((delta, elapsed) => {
    screens?.update(delta, elapsed); mirrors?.update(delta, elapsed)
    const gp = state.scene.userData.guidePulses as THREE.Mesh[] | undefined
    if (gp) { for (const p of gp) { const d = (p as any).userData.guidePulse; if (d) { const s = 0.6 + Math.sin(elapsed * 4 + d.offset) * 0.4; p.scale.setScalar(s); (p.material as THREE.MeshBasicMaterial).opacity = 0.2 + Math.sin(elapsed * 4 + d.offset) * 0.3 } } }
  })
  addFrameCallback((_delta, elapsed) => { codeFlow?.update(elapsed); ceilingFlow?.update(elapsed) })
  addFrameCallback((delta, elapsed) => {
    if (character && orbitCamera) {
      const pos = character.getPosition(); const moved = pos.distanceToSquared(lastCharPos) > 0.0001; lastCharPos.copy(pos)
      if (moved) charMoveTimer = 0.3; else charMoveTimer = Math.max(0, charMoveTimer - delta)
  ...
  ;(window as any).__debugScreens = screens; (window as any).__debugState = state
  ...
  state.renderer.domElement.addEventListener('mousedown', (e: MouseEvent) => { _md = true; _msx = e.clientX; _msy = e.clientY; (window as any).__dragMoved = false })
  state.renderer.domElement.addEventListener('mousemove', (e: MouseEvent) => { if (_md && Math.hypot(e.clientX - _msx, e.clientY - _msy) > 5) (window as any).__dragMoved = true })
  window.addEventListener('mouseup', () => { _md = false })
  state.renderer.domElement.addEventListener('click', (e: MouseEvent) => { if ((window as any).__dragMoved) e.stopImmediatePropagation() }, true)
  window.addEventListener('keydown', onKeyDown)
  state.scene.userData.cleanup = () => {
    state!.renderer.domElement.removeEventListener('click', onCanvasClick); window.removeEventListener('keydown', onKeyDown)
    window.removeEventListener('mouseup', () => { _md = false }); (window as any).__dragMoved = false; _md = false
  }
  startRenderLoop()
...
onBeforeUnmount(() => {
  ...
  codeFlow?.dispose(); ceilingFlow?.dispose(); destroy()
})
</script>
